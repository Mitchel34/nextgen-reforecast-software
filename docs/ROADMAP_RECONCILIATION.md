...
**23 September addendum:** [product objectives and user journeys](PRODUCT_OBJECTIVES_AND_USER_JOURNEYS.md) supersede earlier researcher-input and hosted-service assumptions. Identifier/date/lead entry, automatic acquisition/preparation and the hosted graphical interface now define the intended user experience. The mappings below retain the September 22 engine baseline; planned WEB-01–WEB-07 packages and the [UX protocol](UX_EVALUATION_PROTOCOL.md) add the service/evaluation layer without changing historical evidence or M0–M7/RF identifiers.
...
| 24. Long-term vision | Product objective adopted; complete end-to-end capability planned | RF-001–RF-018 supply the engine/research foundation. The September 23 product direction adds a hosted browser workflow and automatic preparation through proposed WEB-01–WEB-07. Additional formulations and portable checkpoints remain separately qualified capabilities; hosting/funding decisions remain open. |
