...
The [24 September working-plan revision](../docs/WORKING_PLAN.md) separates diagnostic reference admission from release-profile qualification, defines a reproduction candidate before final release, and connects the browser study to an explicit evidence package. Protocol 0.3 changes planning gates without changing the original fixture, numerical criteria or invocation ceilings. No new results accompany this revision.
...
The [first-experiment decision](../docs/FIRST_PHYSICAL_EXPERIMENT.md) admits the known-concern historical baseline for diagnostic reference generation only after build, input and parameter-source admission. Generated reference hashes are then sealed before comparisons consume them. Later coupled parameter qualification has a separate manifest and allowance; baseline agreement does not qualify a corrected release configuration. [C21, C35]
...
New River will provide a second physical domain with two nested outputs, after its fixture and explicit geometry treatments are available. An independent operator will then use an immutable, licensed and accessible reproduction candidate and documented example in a qualified clean environment. The unchanged example and a supported variation with its own linked experiment identity are separate outcomes. The final release follows this evaluation and relevant fixes/retests under the [release sequence](../docs/RELEASE_SEQUENCE.md). Another automated agent on the developer's machine will not count as that external reproduction. [C25]

Separately, the proposed browser study will evaluate the admitted [researcher journeys](../docs/PRODUCT_OBJECTIVES_AND_USER_JOURNEYS.md) using the [UX protocol](../docs/UX_EVALUATION_PROTOCOL.md) and [versioned evidence contract](../docs/UX_EVIDENCE_CONTRACT.md). A study manifest binds software, scenarios, task assignments and evidence levels; retained attempts, assistance, expected blocks, failures and comprehension support reproducible summaries for Q5/Table 5. Namespaced UX_E1–UX_E4 distinguish prototype, synthetic-service, physical-workflow and independent-reproduction evidence from benchmark E01–E09. Descriptive task completion does not establish comparative ease or speed. No user sessions have been conducted for this draft. [C45]
